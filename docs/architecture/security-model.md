...
- There is no server-side session revocation. Deleting an account, resetting a password or changing a password doesn't invalidate JWTs already issued — see [known gaps](../known-gaps.md).
...
- Deleting an item deletes the R2 object named by its stored `fileUrl`. That URL is accepted from the client, and the key isn't checked against the caller's prefix — see [known gaps](../known-gaps.md).
