...
export default function ChaosAnimation() {
  ...
  return (
    <div
      ref={containerRef}
      className="relative h-[280px] max-md:h-[220px] overflow-hidden rounded-2xl border border-border bg-card bg-dots"
    >
      {ICON_SVGS.map((svg, i) => (
        <div
          key={i}
          ref={(el) => { iconsRef.current[i] = el; }}
          className="absolute flex items-center justify-center text-muted-foreground opacity-60 transition-colors hover:text-coral"
